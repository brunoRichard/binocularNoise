...
theBlockData <- list.files(paste0(theDataDirectory,theCompleteParticipants[4]), pattern = "*.mat", full.names = TRUE)
...
```{r}
for (i in 1:64){
  plot(abs(theFFTData[i, 2:200, 9,1]), type = "l")
  abline(v = 31)
}
plot(abs(theFFTData[29, 2:200, 9,1]), type = "l")
plot(abs(theFFTData[30, 2:200, 9,1]), type = "l")
plot(abs(theFFTData[31, 2:200, 9,1]), type = "l")
plot(abs(theFFTData[32, 2:200, 9,1]), type = "l")
```
