...
# Left Eye
for (T in 1:20){
  fIM.Left <- fft(theImagesLeft[,,T])
  fIM.Right <- fft(theImagesRight[,,T])
...
m <- 1.3
S <- 1
stage1.Left <- filteredIM.Left**m
stage1.Right <- filteredIM.Right**m
```
...
```{r}
summedImages <- array(NA, dim = c(stim.Size, stim.Size, nOrientations, nSF, nFrames))
for (T in 1:20){
  for (O in 1:nOrientations){
    for (S in 1:nSF){
      summedImages[,,O,S,T] <- filteredIM.Left[,,O,S,T]**2 + filteredIM.Right[,,O,S,T]**2
    }
  }
}
spatialSum <- apply(summedImages, MARGIN = c(3,4,5), FUN = base::sum)
